...
export class CreateBillPaymentComponent implements OnInit {
  ...
  updatePaymentTransactions() {
    ...
    if (this.openingBalanceAmount > 0) {
      ...
      if (this.advanceAdjusted > 0 && this.paymentAmount == 0) {
        const balanceAmount =
          Number(this.advanceAdjusted) - Number(this.openingBalanceAmount)
        if (balanceAmount >= 0) {
        ...
        if (balanceAmount >= 0) {
          ...
          const balanceAmount =
            Number(this.advanceAdjusted) +
            Number(this.paymentAmount) -
            Number(this.openingBalanceAmount)
          if (balanceAmount >= 0) {
            advanceTransactionAmount = this.advanceAdjusted
            paymentTransactionAmount =
              Number(this.openingBalanceAmount) - Number(this.advanceAdjusted)
            this.advanceAdjusted = 0
            this.paymentAmount =
              Number(this.paymentAmount) - Number(paymentTransactionAmount)
          } else {
            advanceTransactionAmount = this.advanceAdjusted
    ...
    if (billMasterDetails && billMasterDetails.length > 0) {
      ...
      billMasterDetails.forEach((billMaster) => {
        if (this.advanceAdjusted > 0 && this.paymentAmount == 0) {
          const balanceAmount =
            Number(this.advanceAdjusted) - Number(billMaster.toPay)
          if (balanceAmount >= 0) {
          ...
          if (balanceAmount >= 0) {
            ...
            const balanceAmount =
              Number(this.advanceAdjusted) +
              Number(this.paymentAmount) -
              Number(billMaster.toPay)
            if (balanceAmount >= 0) {
              advanceTransactionAmount = this.advanceAdjusted
              paymentTransactionAmount =
                Number(billMaster.toPay) - Number(this.advanceAdjusted)
              this.advanceAdjusted = 0
              this.paymentAmount =
                Number(this.paymentAmount) - Number(paymentTransactionAmount)
            } else {
  ...
  onGetValues() {
    ...
    this.excessAmount = Number(
      this.billAmountDetailsComponent.excessAmount == 0
        ? 0
        : this.billAmountDetailsComponent.excessAmount
    )
  ...
  updateBillPayments() {
    ...
    if (
      this.outstandingBillsComponent &&
      this.outstandingBillsComponent.selectedRows &&
      this.outstandingBillsComponent.selectedRows.length
    ) {
