...
export class CreateBillComponent implements OnInit {
  ...
  @ViewChild(UnbilledConsumptionComponent, { static: true }) unbilledConsumptionComponent: UnbilledConsumptionComponent;
  @ViewChild(CreateBillToolbarComponent, { static: true }) createBillToolbarComponent: CreateBillToolbarComponent;
  ...
  onSaveBills(event: boolean) {
    ...
    if (billMasterDetails != null && billMasterDetails.length) {
      this.billSettlementService.saveUnBilledConsumptions(billMasterDetails).subscribe({
        next: (response) => {
          if (response) {
            this.showSpinner = false;
            this.notificationMessage('Bill saved successfully', 'green-snackbar');
  ...
  reset(reset: boolean = true, billMasterDetails: BillMaster[] = []) {
    let existingUnBilledConsumptions = this.unbilledConsumptionComponent.unBilledConsumptions;
    if (!reset && existingUnBilledConsumptions && existingUnBilledConsumptions.length) {
      billMasterDetails.forEach(billMaster => {
        existingUnBilledConsumptions = existingUnBilledConsumptions.filter(x => x.unitId != billMaster.unitId);
      });
      this.unbilledConsumptionComponent.unBilledConsumptions = existingUnBilledConsumptions;
    } else if (reset || (existingUnBilledConsumptions && billMasterDetails && existingUnBilledConsumptions.length == billMasterDetails.length)) {
  ...
  onGenerate(manageParams: ManageParams) {
    const billGenerateParameter: BillGenerateParameter = {
      ...
      billType: manageParams.billType ? manageParams.billType : BillType.NormalBill,
      clientId: manageParams.clientId,
      tenantIds: manageParams.tenantIds
    };
    this.showSpinner = true;
    this.billSettlementService.generateBillDetails(billGenerateParameter).subscribe({
