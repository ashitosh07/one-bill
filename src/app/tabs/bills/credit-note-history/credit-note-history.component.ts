...
export class CreditNoteHistoryComponent implements OnInit {
  ...
  @ViewChild(CreditNoteHistoryDetailsComponent, { static: true })
  creditNoteHistoryDetailsComponent: CreditNoteHistoryDetailsComponent
  @ViewChild(CreditNoteHistoryToolbarComponent, { static: true })
  creditNoteHistoryToolbarComponent: CreditNoteHistoryToolbarComponent
  ...
  createColumnNames() {
    this.columnNames = [
      {
        name: this.creditNoteDateCoulmnName,
        property: 'creditNoteDateLocal',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.creditNoteNumberCoulmnName,
        property: 'creditNoteNumber',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.billNumberCoulmnName,
        property: 'billNumber',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.billDateCoulmnName,
        property: 'billDateLocal',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.accountNumberColumnName,
        property: 'accountNumber',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.ownerNameColumnName,
        property: 'ownerName',
        visible: true,
        isModelProperty: true,
      },
      {
        name: this.billAmountLocalColumnName,
      ...
      {
        name: this.creditNoteAmountColumnName,
        property: 'creditNoteAmountLocal',
        visible: true,
        isModelProperty: true,
        columnAlign: { 'text-align': 'right' },
      },
  ...
  onSearch(manageParams: ManageParams) {
    ...
    this.billSettlementService
      .getCreditNoteDetails(manageParams)
      .subscribe((creditNotes) => {
        if (creditNotes && creditNotes.length) {
          ...
          creditNotes.forEach((element) => {
            element.billAmountLocal = this.currencyPipe.transform(
              element.billAmount,
              this.currencyFormat.toString(),
              true,
              this.roundFormat
            )
            element.creditNoteAmountLocal = this.currencyPipe.transform(
              element.creditNoteAmount,
              this.currencyFormat.toString(),
              true,
              this.roundFormat
            )
            element.billDateLocal = this.datePipe.transform(
              element.billDate,
              this.dateFormat
            )
            element.creditNoteDateLocal = this.datePipe.transform(
              element.creditNoteDate,
              this.dateFormat
            )
          ...
          creditNotes.sort((a, b) =>
            a.creditNoteDateLocal.localeCompare(b.creditNoteDateLocal)
          )
          this.creditNotes = creditNotes
          this.billAmount = this.currencyPipe.transform(
            billAmount,
            this.currencyFormat.toString(),
            true,
            this.roundFormat
          )
          this.creditNoteAmount = this.currencyPipe.transform(
            creditNoteAmount,
            this.currencyFormat.toString(),
            true,
            this.roundFormat
          ...
          if (manageParams.fromDate != '' && manageParams.toDate != '') {
            this.message =
              'Period : ' +
              this.datePipe.transform(manageParams.fromDate, this.dateFormat) +
              ' to ' +
              this.datePipe.transform(manageParams.toDate, this.dateFormat)
          }
        } else {
          this.creditNotes = []
        }
  ...
  onViewDataRow(row: CreditNote) {
    if (row) {
      ...
      row.creditNoteTransactions.forEach((element) => {
        ...
        element.fixedAmountLocal = this.currencyPipe.transform(
          element.fixedAmount,
          this.currencyFormat.toString(),
          true,
          this.roundFormat
    ...
    this.dialog
      .open(CreditNoteDetailsComponent, { data: row })
      .afterClosed()
      .subscribe()
  }
  ...
  downloadSummary(creditNotes: CreditNote[]) {
    ...
    secondTableRows.push({
      label: 'Total Amount',
      value0: totalBillAmount,
      value1: totalCreditNoteAmount,
    })
  ...
  getSummaryReport(
    ...
    if (img && data && this.imageProperties && this.imageProperties.length) {
      const imageProperty: ImageProperty = this.imageProperties.find(
        (x) => x.imageType.trim().toLowerCase() === 'landscape'
      )
      if (imageProperty) {
        img.onload = function () {
          pdf.addImage(
    ...
    pdf.setFontSize(14)
    pdf.text(
      'Credit Note History ' + this.billPeriod,
      pdf.internal.pageSize.width / 2 - 120,
      startY + 70
  ...
  downloadReport(creditNote: CreditNote) {
    ...
    for (let a = 0; a < creditNote.creditNoteTransactions.length; a++) {
      if (
        creditNote.creditNoteTransactions[a] &&
        !creditNote.creditNoteTransactions[a].taxId
      ) {
        const amount = this.currencyPipe.transform(
          Number(creditNote.creditNoteTransactions[a].creditNoteAmount),
          this.currencyFormat,
          true,
          this.roundFormat
        )
        ...
        firstTableRows.push(row)
        totalAmount += Number(
          creditNote.creditNoteTransactions[a].creditNoteAmount
        )
        row = []
      } else {
        vatAmount += Number(
    ...
    fourthTableRows.push({
      ...
      value: this.currencyPipe.transform(
        Number(creditNote.previousDueAmount),
        this.currencyFormat.toString(),
        true,
        this.roundFormat
      ),
  ...
  getReport(
    ...
    pdf.text(
      'PO Box ' + data.client.addresses[0].zipPostalCode,
      startX,
      startY + 50
    )
    pdf.text(
      data.client.addresses[0].address1 +
        ',' +
        data.client.addresses[0].country,
      startX,
      startY + 60
    )
    pdf.text('Phone: ' + data.client.phoneNo, startX, startY + 70)
    pdf.text(
      'Email: ' + data.client.email + ',' + ' Web: ' + data.client.website,
      startX,
      startY + 80
    )
    ...
    pdf.setFontSize(9)
    pdf.text(
      'Account Title: LOGIC UTILITIES DISTRICT COOLING SERVICES L.L.C',
      startX,
      thirdTableEndY + 40
    )
    pdf.text('Account #: 019100074892', startX, thirdTableEndY + 50)
    pdf.text(
      'IBAN #: AE54 0330 0000 1910 0074 892',
      startX,
      thirdTableEndY + 60
    )
    pdf.text(
      'Bank Name: MASHREQ BANK - DUBAI MALL BRANCH',
      startX,
      thirdTableEndY + 70
    )
    ...
    const fourthTableEndY = Number(
      this.decimalPipe.transform(pdf[autoTable].previous.finalY, '1.0-0')
    )
    pdf.setTextColor(255, 0, 0)
    pdf.text(
      'Note: A Late Fee will be applied after the due date. A Default Payment \nPenalty will be added to any unit that is 45 days or more in arrears.',
      startX + 310,
      fourthTableEndY + 10
    )
    ...
    pdf.rect(
      ...
      fourthTableEndY + 30,
      280,
      pageHeight - 80 - (fourthTableEndY + 30),
      'FD'
    )
    ...
    pdf.text(
      'For any enquiries, write to us enquiry@logicutilities.com \nor Logon to www.logicutilites.com',
      startX,
      pageHeight - 60
    )
    pdf.setTextColor(255, 0, 0)
    ...
    pdf.text(
      'You can log onto www.logicutilites.com to pay your invoices \nTerms & Conditions for Supply of Utility Services',
      startX + 330,
      pageHeight - 40
    )
