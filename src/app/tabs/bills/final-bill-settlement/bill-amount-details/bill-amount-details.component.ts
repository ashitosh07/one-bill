...
import {
  convertToNumber,
  getClientDataFormat,
} from 'src/app/tabs/shared/utilities/utility'
...
export class BillAmountDetailsComponent implements OnInit {
  ...
  set dataSource(value: BillSettlement) {
    ...
    if (value && !this.isHide) {
      if (value.accountHeads && value.accountHeads.length) {
        ...
        this.accountHeadData = value.accountHeads.filter(
          (x) => x.accountHeadName !== 'VAT'
        )
      }
      if (value.outstandingBills && value.outstandingBills.length) {
        this.billAmount = 0

        value.outstandingBills.forEach(
          (x) => (this.billAmount += Number(x.toPay))
        )
        this.toPay = this.decimalPipe
          .transform(this.billAmount, this.roundFormat.toString())
          .replace(/,/g, '')
        this.roundOff =
          this.total =
          this.billAmount =
    ...
    if (value && value.openingBalance) {
      ...
      if (this.total > this.previousAdvanceAmount) {
        ...
        this.toPay = this.decimalPipe
          .transform(
            this.total - this.previousAdvanceAmount,
            this.roundFormat.toString()
          )
          .replace(/,/g, '')
    ...
    this.toPay = this.decimalPipe
      .transform(this.toPay, this.roundFormat.toString())
      .replace(/,/g, '')
  }
    ...
    if (value && value.length) {
      ...
      if (this.total) {
        if (this.previousAdvanceAmount > 0) {
          this.roundOff = this.total = Number(
            this.decimalPipe
              .transform(
                this.total + this.billAmount,
                this.roundFormat.toString()
  ...
  @ViewChild(TableStructureComponent, { static: false })
  tableStructureComponent: TableStructureComponent

  constructor(private dialog: MatDialog, private decimalPipe: DecimalPipe) {}
  ...
  createGridColumns(): any {
    this.columns = [
      {
        name: this.accountHeadColumnName,
  ...
  addNewItem() {
    this.dialog
      .open(AddNewAccountHeadComponent, { data: this.accountHeadData })
      .afterClosed()
      .subscribe((accountHead: AccountHead) => {
        if (accountHead) {
          ...
          if (this.accountHeads) {
            ...
            this.toPay = this.decimalPipe
              .transform(this.total, this.roundFormat.toString())
              .replace(/,/g, '')
          }
        }
      })
  ...
  onAdvanceAdjustedChange(value: number) {
    if (Number(this.toPay) > this.total) {
      ...
      this.excessAmount = Number(
        this.decimalPipe
          .transform(
            Number(this.toPay) - this.total,
            this.roundFormat.toString()
          )
          .replace(/,/g, '')
      )
  ...
  onToPayAmountChange(value: number) {
    ...
    if (
      convertToNumber(this.toPay) + convertToNumber(this.advanceAdjusted) >
      convertToNumber(this.total)
    ) {
      this.labelName = 'Excess Amount'
      this.excessAmount = Number(
        this.decimalPipe
          .transform(
            convertToNumber(this.toPay) +
              convertToNumber(this.advanceAdjusted) -
              convertToNumber(this.total),
            this.roundFormat.toString()
          )
          .replace(/,/g, '')
  ...
  onRoundOffAmountChange(value: number) {
    if (value > 0) {
      this.toPay = this.decimalPipe
        .transform(
          Number(this.total) + Number(value),
          this.roundFormat.toString()
        )
        .replace(/,/g, '')
    } else {
  ...
  onCalculateVAT(accountHeads: AccountHead[]) {
    if (accountHeads && accountHeads.length) {
      ...
      if (accountHead) {
        if (
          accountHead.formulaDescription === 'GrandTotal' &&
          accountHead.operatorDescription === 'Percentage'
        ) {
          this.vatAmount = Number(
            this.decimalPipe.transform(
              (Number(this.toPay) * Number(accountHead.fixedAmount)) / 100,
              this.roundFormat
            )
          )
        } else if (
          ...
          this.vatAmount = Number(
            this.decimalPipe.transform(
              Number(this.toPay) + Number(accountHead.fixedAmount),
              this.roundFormat
            )
          )
        }
    ...
    this.toPay = this.decimalPipe.transform(
      Number(this.toPay) + this.vatAmount,
      this.roundFormat
    )
  }
  ...
  onChangeAdjustWithAdvance(event: any) {
    ...
    if (this.isAdjustWithAdvance) {
      if (this.total >= this.previousAdvanceAmount) {
        ...
        this.toPay = this.decimalPipe
          .transform(
            Number(total - previousAdvanceAmount),
            this.roundFormat.toString()
          )
          .replace(/,/g, '')
      } else {
      ...
      this.toPay = this.decimalPipe
        .transform(Number(this.total), this.roundFormat.toString())
        .replace(/,/g, '')
    }
  }
}
