...
import { BillType, getClientDataFormat, stringIsNullOrEmpty } from '../../shared/utilities/utility';
...
export class FinalBillSettlementComponent implements OnInit {
  ...
  onGetBillSettlementDetails(manageParams: ManageParams) {
    ...
    this.billSettlementService.getBillSettlementDetails(manageParams).subscribe({
      next:
        (billSettlement) => {
          this.showSpinner = false;
          this.billSettlement = billSettlement;
          this.isData = billSettlement.outstandingBills && billSettlement.outstandingBills.length ? true : false;
        },
      error: (err) => {
        this.showSpinner = false;
      }
  ...
  onGetOutStandingBillDetails(manageParams: ManageParams) {
    this.isData = false;
    this.manageParams = manageParams;
    this.billSettlementService.getOutStandingBillDetails(manageParams).subscribe(
      billSettlement => {
        this.billSettlement = billSettlement;
        this.isData = billSettlement.outstandingBills && billSettlement.outstandingBills.length ? true : false;
      });
  ...
  onSaveUnbilledConsumption(event: boolean) {
    const billMasterDetails: BillMaster[] = this.billSettlement.unbBilledConsumptions;
  ...
  downloadReport(payment: Payment) {
    ...
    const balanceAmount = paidAmount > payment.outStandingAmount ? 0 : payment.outStandingAmount - paidAmount;
