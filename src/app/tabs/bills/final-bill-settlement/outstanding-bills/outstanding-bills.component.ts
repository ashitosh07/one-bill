...
export class OutstandingBillsComponent implements OnInit {
  ...
  createGridColumns(): any {
    this.columns = [
      ...
      { name: this.dateColumnName, property: 'billDateLocal', visible: true, isModelProperty: true },
      { name: this.billNumberConsumptionColumnName, property: 'billNumber', visible: true, isModelProperty: true },
      { name: this.unitColumnName, property: 'unitNumber', visible: true, isModelProperty: true },
      { name: this.amountColumnName, property: 'netAmountLocal', visible: true, isModelProperty: true, columnAlign: { 'text-align': 'right' } },
      ...
      { name: this.toPayColumnName, property: 'toPayLocal', visible: true, isModelProperty: true, columnAlign: { 'text-align': 'right' } }] as ListColumn[];
  }
  ...
  updateDataFormats(outstandingBills: BillMaster[]) {
    if (outstandingBills) {
      ...
      outstandingBills.forEach(billMaster => {
        billMaster.billDateLocal = this.date.transform(billMaster.billDate.toString(), this.format.toString());
        ...
        billMaster.netAmountLocal = this.currencyPipe.transform(billMaster.billAmount - billMaster.creditNoteAmount, this.currency.toString(), true, this.roundFormat);
        billMaster.creditNoteAmountLocal = this.currencyPipe.transform(billMaster.creditNoteAmount, this.currency.toString(), true, this.roundFormat);
        billMaster.toPayLocal = this.currencyPipe.transform(billMaster.toPay, this.currency.toString(), true, this.roundFormat);
        billMaster.paidLocal = this.currencyPipe.transform(billMaster.paid, this.currency.toString(), true, this.roundFormat);
      })
    }
