...
export class UnbilledConsumptionComponent implements OnInit {
  ...
  set dataSource(value: BillSettlement) {
    if (value && value.unbBilledConsumptions) {
      this.unBilledConsumptions = this.updateDataFormats(value.unbBilledConsumptions);
      const succeedCount: number = this.unBilledConsumptions.filter(x => x.status === 'Succeed').length ?? 0;
      const failedCount: number = this.unBilledConsumptions.filter(x => x.status === 'Failed').length ?? 0;
  ...
  createColumnNames() {
    this.columnNames = [
      ...
      { name: this.consumptionChargeColumnName, property: 'consumptionCharge', columnAlign: { 'text-align': 'right' } }] as ListColumn[];
  }
  ...
  updateDataFormats(unBilledConsumptions: BillMaster[]) {
    if (unBilledConsumptions) {
      ...
      unBilledConsumptions.forEach(billMaster => {
        billMaster.fromDateLocal = this.date.transform(billMaster.fromDate.toString(), this.format.toString());
        billMaster.toDateLocal = this.date.transform(billMaster.toDate.toString(), this.format.toString());
        billMaster.dueDateLocal = this.date.transform(billMaster.dueDate.toString(), this.format.toString());
