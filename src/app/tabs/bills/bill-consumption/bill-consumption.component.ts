...
export class BillConsumptionComponent implements OnInit {
  ...
  @ViewChild(BillConsumptionDetailsComponent, { static: true })
  billConsumptionDetailsComponent: BillConsumptionDetailsComponent
  @ViewChild(BillConsumptionToolbarComponent, { static: true })
  billConsumptionToolbarComponent: BillConsumptionToolbarComponent
  ...
  getConsumptionAlertRanges() {
    ...
    if (manageParams) {
      this.billSettlementService
        .getConsumptioSavedAlertDetails(manageParams)
        .subscribe((consumptionAlerts: ConsumptionAlertRange[]) => {
  ...
  onSearch(manageParams: ManageParams) {
    ...
    this.consumptionRoundOffFormat = getClientDataFormat(
      'ConsumptionRoundOff',
      parseInt(manageParams.utilityTypeId)
    )
    ...
    this.billSettlementService
      .getConumptionAlertDetails(manageParams)
      .subscribe((bills: Bill[]) => {
        if (bills && bills.length) {
          ...
          bills.forEach((element) => {
            ...
            element.consumption = Number(
              this.decimalPipe
                .transform(
                  element.consumption,
                  this.consumptionRoundOffFormat.toString()
                )
                .replace(',', '')
            )
            element.consumptionLocal =
              element.consumption.toFixed(this.consumptiondecimalPlaces) +
              ' ' +
              element.consumptionUnit
            element.consumptionDifference = Number(
              this.decimalPipe
                .transform(
                  element.consumptionDifference,
                  this.consumptionRoundOffFormat.toString()
                )
                .replace(',', '')
            )
            element.consumptionDifferenceLocal =
              element.consumptionDifference.toFixed(
                this.consumptiondecimalPlaces
              ) +
              ' ' +
              element.consumptionUnit
            element.averageConsumption = Number(
              this.decimalPipe
                .transform(
                  element.averageConsumption,
                  this.consumptionRoundOffFormat.toString()
                )
                .replace(',', '')
            )
            element.averageConsumptionLocal =
              element.averageConsumption.toFixed(
                this.consumptiondecimalPlaces
              ) +
              ' ' +
              element.consumptionUnit
            element.consumptionDifferencePercentageLocal =
              element.consumptionDifferencePercentage.toFixed(
                this.consumptiondecimalPlaces
              )
          })
          ...
          this.isData = true
          this.message =
            manageParams.consumptionType +
            ' of ' +
            manageParams.utilityType +
            ' in comparison with ' +
            manageParams.percentage +
            '% buffer'
        } else {
          this.billConsumptionDetailsComponent.failedBills = []
          this.isData = false
  ...
  onReset() {
    ...
    this.billConsumptionDetailsComponent.failedBills = []
    this.billConsumptionToolbarComponent.form.controls.consumptionTypeId.setValue(
      0
    )
    this.billConsumptionToolbarComponent.form.controls.utilityTypeId.setValue(0)
    this.billConsumptionToolbarComponent.billPeriodForm.controls.fromBillPeriodId.setValue(
      0
    )
    this.billConsumptionToolbarComponent.billPeriodForm.controls.toBillPeriodId.setValue(
      0
    )
    this.getConsumptionAlertRanges()
  }
  ...
  onSave() {
    const consumptionTypeId =
      this.billConsumptionToolbarComponent.form.controls.consumptionTypeId.value
    const utilityTypeId =
      this.billConsumptionToolbarComponent.form.controls.utilityTypeId.value
    ...
    if (optionId == '1') {
      fromBillPeriodId =
        this.billConsumptionToolbarComponent.billPeriodForm.controls
          .fromBillPeriodId.value
      toBillPeriodId =
        this.billConsumptionToolbarComponent.billPeriodForm.controls
          .toBillPeriodId.value
      percentage =
        this.billConsumptionToolbarComponent.billPeriodForm.controls
          .percentageBillPeriod.value
    } else {
      fromDate = new Date(
        this.datePipe.transform(
          this.billConsumptionToolbarComponent.fromDate,
          'yyyy-MM-dd'
        )
      )
      startDate = new Date(
        this.datePipe.transform(
          this.billConsumptionToolbarComponent.startDate,
          'yyyy-MM-dd'
        )
      )
      endDate = new Date(
        this.datePipe.transform(
          this.billConsumptionToolbarComponent.endDate,
          'yyyy-MM-dd'
        )
      )
    ...
    if (consumptionTypeId && utilityTypeId) {
      ...
      this.billSettlementService
        .saveConsumptionRange(consumptionRange)
        .subscribe({
          next: (response) => {
            if (response) {
              this.notificationMessage(
                'Consumption range saved successfully',
                'green-snackbar'
              )
              this.onReset()
            } else {
              this.notificationMessage(
                'Consumption range save failed',
                'red-snackbar'
              )
            }
          },
  ...
  onExport() {
    if (
      this.billConsumptionDetailsComponent.failedBills &&
      this.billConsumptionDetailsComponent.failedBills.length > 0
    ) {
      this.getJsonData()
