...
export class CreateBillHeadTransactionComponent implements OnInit {
  ...
  subject$:ReplaySubject<BillHeadTransaction[]>=new ReplaySubject<BillHeadTransaction[]>(1);
  ...
  columns:ListColumn[]=[
    ...
    {name:'Bill Head Name' ,property:'accountHeadName',visible:true,isModelProperty:true},
    {name:'Formula',property:'formula',visible:true,isModelProperty:true},
  ...
  constructor(private billHeadTransactionService:BillHeadTransactionService,private fb: FormBuilder,
    private masterService: MasterService,
    private snackbar:MatSnackBar,private metadataService:MetadataService) { }
  ...
  ngOnInit() {
    ...
    this.billHeadTransactionService.getBillHeadTransactions().
    subscribe((billHeadTransactions:BillHeadTransaction[])=>{
      billHeadTransactions=billHeadTransactions.
      map(billHeadTransaction => new BillHeadTransaction(billHeadTransaction));
      this.subject$.next(billHeadTransactions);
    });

    this.billHeadTransactionService.getBillSettings().subscribe((data: Master[]) => {
      if(data) {
    ...
    this.form.controls.billSettingsName.valueChanges.subscribe(newBillSettings => {
      this.filteredBillSettings = this.filterBillSettings(newBillSettings);
    });
    this.form.controls.accountHeadName.valueChanges.subscribe(newAccountHeadName => {
  ...
  filterBillHeads(name: string) {
    return this.metadataBillHeads.filter(billHeads =>
  ...
  createBillHeadTransaction(){
    ...
    if( this.defaults.id !=0 )
    {
      ...
      this.billHeadTransactionService.createBillHeadTransaction(this.defaults).
      subscribe((billHeadTransaction:BillHeadTransaction)=>{
        this.snackbar.open('created Bill head Transaction Successfully',null,{
          duration:5000,
          verticalPosition:'top',
          horizontalPosition:'center',
          panelClass:['green-snackbar'],
        });
        this.billHeadTransactions.unshift(new BillHeadTransaction(billHeadTransaction));
  ...
  updateBillHeadTransactionFromForm(defaults){
    this.billHeadTransactionService.updateBillHeadTransactionById(defaults.id,defaults).
    subscribe((billHeadTransaction:BillHeadTransaction)=>{
     ...
     const index=this.billHeadTransactions.
     findIndex((existingbillHeadTransactions)=>existingbillHeadTransactions.id===billHeadTransaction.id);
   this.billHeadTransactions[index]=new BillHeadTransaction(billHeadTransaction);
   
   this.billHeadTransactionService.getBillHeadTransactions().subscribe((billHeadTransactions:BillHeadTransaction[])=>{
    billHeadTransactions=billHeadTransactions.map(billHeadTransaction =>new BillHeadTransaction(billHeadTransaction));
    this.subject$.next(billHeadTransactions);
    this.form.reset();
  });
     })
...
this.billHeadTransactionService.deleteBillHeadTransactionById(billHeadTransaction.id).subscribe(() => {
this.billHeadTransactions.splice(this.billHeadTransactions.
  findIndex((existingbillHeadTransactions) => existingbillHeadTransactions.id === billHeadTransaction.id), 1);
this.subject$.next(this.billHeadTransactions);
})
}
