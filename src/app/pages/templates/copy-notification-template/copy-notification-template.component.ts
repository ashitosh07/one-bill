...
@Component({
  selector: 'fury-copy-notification-template',
  templateUrl: './copy-notification-template.component.html',
  styleUrls: ['./copy-notification-template.component.scss']
})
export class CopyNotificationTemplateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private dialogRef: MatDialogRef<CopyNotificationTemplateComponent>,
    ...
    this.dialog.open(UserConfirmationDialogComponent, {
      ...
      if (response) {
        if (this.selectedClients && this.selectedClients.length) {
          ...
          this.templateService.copyNotificationTemplates(copyContents).subscribe({
            next: (response: boolean) => {
              if (response) {
                this.notificationMessage('Notification Templates copied successfully.', 'green-snackbar');
              } else {
                this.notificationMessage('Notification Templates copy failed.', 'red-snackbar');
              }
              this.dialogRef.close();
            },
            error: (err) => {
