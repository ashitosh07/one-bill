...
export class DashboardComponent implements OnInit {
  ...
  // Invoice variables start
  invoiceData;
  invoiceOptions: BarChartWidgetOptions = {
  ...
  pieDatas=[12,31,67];
  pieLabels=['one','two','three'];
  
  invoicePieData;
  invoicePieOptions: DonutChartWidgetOptions = {
    title: 'Invoice Data',
    subTitle: ''
  };
  // Invoice variables end
  ...
  ngOnInit() {
      ...
      this.dashboardService.getDatas().subscribe((data: any) => {
        if(data) {
        this.invoiceData=this.dashboardService.toInvoiceData(data);   
        this.invoicePieData=this.dashboardService.toInvoicePieData(data);   
        } else {
