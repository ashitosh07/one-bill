...
export class DashboardService {
  ...
  toInvoiceData(chartData) {

    let labels = [];
    let datas = [];
    chartData.forEach(element => {

      labels.push(element['dataType']);
      datas.push(element['dataCount']);
    })
    return {
  ...
  toInvoicePieData(chartData) {
    return {
      labels: chartData.map(data => data.dataType),
      datasets: [
        {
          data: chartData.map(data => data.dataCount),
